...
class MultiProcessor(object):
    ...
    def __init__(self, target_func, num_processes=mp.cpu_count(), **kwargs):
        ...
        self.total_jobs = 0             # tracks the total number of jobs
        self.jobs = mp.JoinableQueue()  # initiate queue that can be extended
        self.results = mp.Queue()       # create a Queue for results

        tup = (self.jobs, self.results)
        self.num_processes = num_processes

        # Generate a list of processes with length num_processes
